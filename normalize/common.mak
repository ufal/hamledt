...
default_ud_postprocessing:
	...
	( for i in *.conllu ; do if [ -s $$i ] ; then echo $$i ; cp $$i $$i.debug ; udapy -s $(UDAPISCEN) < $$i > fixed.conllu ; mv fixed.conllu $$i ; mv $$i $(UDDIR)/UD_$(UDNAME) ; else rm $$i ; fi ; done ) |& tee udapi.log
