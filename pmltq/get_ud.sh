...
# Usage: $0 [--release 25 --only cs-cac] # limiting it to one treebank, identified by its target name
udrel="25" # default: UD 2.5
POSITIONAL=()
while [[ $# -gt 0 ]]
do
key="$1"
...
for tbkpath in $uddir/*-ud$udrel* ; do
  ...
  if [[ -z "$ONLY" ]] || [[ "$withoutud" == "$ONLY" ]] ; then
    if [ "$withoutud" != "ar-nyuad" ] && [ "$withoutud" != "en-esl" ] && [ "$withoutud" != "fr-ftb" ] && [ "$withoutud" != "gun-dooley" ] && [ "$withoutud" != "qhe-hiencs" ] && [ "$withoutud" != "ja-bccwj" ] && [ "$withoutud" != "ja-ktc" ] ; then
