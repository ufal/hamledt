...
my $RELEASE = $ARGV[0]; # e.g. "2.3"
if(!defined($RELEASE) || $RELEASE !~ m/^[1-9][0-9]*\.[1-9][0-9]*$/)
{
    die('The first argument must be the release number');
}
my $CRELEASE = $RELEASE; # compact string for HamleDT path, e.g. '14' for 'cs-ud14' (Czech in release 1.4)
$CRELEASE =~ s/\.//;
